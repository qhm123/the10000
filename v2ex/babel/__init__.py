...
class Member(db.Model):
    ...
    l10n = db.StringProperty(default='zh-Hans')
...
class Hours(db.Model):
    ...
    mission = db.StringProperty(required=True, default='')
    member = db.ReferenceProperty(Member)
    member_num = db.IntegerProperty(indexed=True)
    edits = db.IntegerProperty(indexed=False, default=1)
    created = db.DateTimeProperty(auto_now_add=True)
    last_modified = db.DateTimeProperty(auto_now=True)
    last_addtime = db.DateTimeProperty(auto_now_add=True)
    last_addhours = db.IntegerProperty(default=0)
