...
class HomeHandler(webapp.RequestHandler):
	...
	if q.count() == 1:
            member1 = q[0]
            q = db.GqlQuery("SELECT * FROM Note WHERE member = :1 ORDER BY last_modified DESC", member1)
            try:
                notes_count = q.count()
            except:
                q = db.GqlQuery("SELECT * FROM Note WHERE member = :1 ORDER BY created DESC", member1)
                notes_count = q.count()
            if (notes_count > 0):
                template_values['notes'] = [note for note in q if note.title not in ['About', 'FAQ', 'Mission', 'Blog', 'Advertise']]

            hours = db.GqlQuery("SELECT * FROM Hours WHERE member = :1", member1)
            if hours.count() == 1:
                hours1 = hours[0]
                template_values['hours'] = hours1
