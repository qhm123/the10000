...
class MemberHandler(webapp.RequestHandler):
    def get(self, member_username):
        ...
        if one is not False:
            # 日记和hours
            q = db.GqlQuery("SELECT * FROM Note WHERE member = :1 ORDER BY last_modified DESC", one)
            try:
                notes_count = q.count()
            except:
                q = db.GqlQuery("SELECT * FROM Note WHERE member = :1 ORDER BY created DESC", one)
                notes_count = q.count()
            if (notes_count > 0):
                template_values['notes'] = q

            q = db.GqlQuery("SELECT * FROM Hours WHERE member = :1", one)
            if q.count() == 1:
                template_values['hours'] = q[0]
...
class SettingsHandler(webapp.RequestHandler):
    def get(self):
        ...
        if (member):
            q = db.GqlQuery("SELECT * FROM Hours WHERE member = :1", member)
            if q.count() == 1:
                hours = q[0]
...
class HoursMissionHandler(webapp.RequestHandler):
    def get(self):
        pass
    ...
    def post(self):
        ...
        if (member):
            ...
            if q.count() < 1:
                # 新建
                hours = Hours(mission=mission)
                hours.member = member
                hours.member_num = member.num
                hours.put()
            else:
                if q.count() == 1:
                    hours = q[0]
                    hours.mission = mission
                    hours.edits += 1
                    hours.put()
        self.redirect('/settings')

class ChangeHoursHandler(webapp.RequestHandler):
    def get(self):
        pass
    ...
    def post(self):
        ...
        if member:
            ...
            if q.count() == 1:
                hours = q[0]
                delta = datetime.datetime.utcnow() - hours.last_addtime
                logging.info("now %s." % datetime.datetime.now())
                logging.info("utcnow %s." % datetime.datetime.utcnow())
                logging.info("last_addtime %s." % hours.last_addtime)
                delta = delta.seconds / 3600
                logging.info("delta: %d." % delta)
                logging.info("- %d." % (value - hours.value,))
                if (value >= hours.value) and (delta >=  value - hours.value):
                    hours.value = value
                    hours.last_addtime = datetime.datetime.now()
                    hours.last_addhours = value - hours.value
                    hours.adds += 1
                    hours.put()
                    ok = True
        result = json.dumps({"ok": ok, "value": value})
...
def main():
    ...
    ('/settings/mission', HoursMissionHandler),
    ('/block/(.*)', MemberBlockHandler),
    ('/unblock/(.*)', MemberUnblockHandler),
    ('/changehours', ChangeHoursHandler)
    ],
                                         debug=True)
